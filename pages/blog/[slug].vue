<template>
  <div>
    <!-- Hero Section -->
    <section class="py-12 bg-gradient-to-br from-secondary-900 to-primary-900">
      <UiContainer size="lg">
        <div class="max-w-4xl">
          <NuxtLink to="/blog" class="inline-flex items-center text-primary-100 hover:text-white mb-6 transition-colors">
          ...
          <div class="flex flex-wrap items-center gap-6 text-primary-100">
            <div class="flex items-center gap-2">
              <div class="w-10 h-10 bg-primary-700 rounded-full" />
    ...
    <section class="bg-secondary-50">
      <UiContainer size="lg">
        <div class="aspect-video bg-secondary-300 rounded-2xl flex items-center justify-center">
          <span class="text-secondary-500 text-lg">Featured Image</span>
        </div>
      </UiContainer>
    </section>
          ...
          <div class="lg:col-span-2">
            <article class="prose prose-lg max-w-none">
              ...
              <div v-for="(section, index) in post.content" :key="index" class="mb-8">
                <h2 class="text-2xl font-bold text-secondary-900 mb-4">{{ section.title }}</h2>
                <p class="text-secondary-600 leading-relaxed mb-4">{{ section.text }}</p>
                <ul v-if="section.list" class="list-disc list-inside space-y-2 text-secondary-600">
              ...
              <div class="bg-primary-50 rounded-xl p-6 mt-12">
                <h3 class="text-lg font-bold text-secondary-900 mb-4">Key Takeaways</h3>
                <ul class="space-y-2">
                  <li v-for="(takeaway, index) in post.takeaways" :key="index" class="flex items-start gap-2">
                    ...
                    <span class="text-secondary-700">{{ takeaway }}</span>
                  </li>
                </ul>
              </div>
            </article>
            ...
            <div class="flex items-center justify-between mt-12 pt-8 border-t border-secondary-200">
              <div class="flex items-center gap-4">
                <span class="text-secondary-600 font-medium">Share this article:</span>
                <div class="flex gap-2">
                  <button class="w-10 h-10 bg-secondary-100 rounded-full flex items-center justify-center hover:bg-primary-100 hover:text-primary-600 transition-colors">
            ...
            <!-- Author Bio -->
            <div class="mt-12 p-6 bg-secondary-50 rounded-xl">
              <div class="flex items-start gap-4">
                <div class="w-16 h-16 bg-primary-200 rounded-full flex-shrink-0" />
                <div>
                  <h3 class="text-lg font-bold text-secondary-900 mb-1">{{ post.author }}</h3>
                  <p class="text-primary-600 text-sm mb-2">{{ post.authorRole }}</p>
                  <p class="text-secondary-600 text-sm">{{ post.authorBio }}</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div class="lg:col-span-1">
            ...
            <div class="sticky top-24">
              <div class="bg-secondary-50 rounded-xl p-6 mb-8">
                <h3 class="text-lg font-bold text-secondary-900 mb-4">Table of Contents</h3>
                <ul class="space-y-2">
                  <li v-for="(section, index) in post.content" :key="index">
                    <a :href="`#section-${index}`" class="text-secondary-600 hover:text-primary-600 text-sm transition-colors">
                      {{ section.title }}
                    </a>
                  </li>
                </ul>
              ...
              <!-- Related Posts -->
              <div>
                <h3 class="text-lg font-bold text-secondary-900 mb-4">Related Articles</h3>
                <div class="space-y-4">
                  <div v-for="(related, index) in relatedPosts" :key="index" class="flex gap-3">
                    <div class="w-16 h-16 bg-secondary-200 rounded flex-shrink-0" />
                    <div>
                      <h4 class="text-sm font-medium text-secondary-900 hover:text-primary-600 cursor-pointer line-clamp-2">
                        {{ related.title }}
                      </h4>
                      <p class="text-xs text-secondary-500">{{ related.date }}</p>
                    </div>
                  </div>
                </div>
              </div>
    ...
    <section class="py-16 bg-secondary-50">
      <UiContainer size="lg">
        <h2 class="text-2xl font-bold text-secondary-900 mb-8">More Articles</h2>
        <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
          <UiCard v-for="(article, index) in moreArticles" :key="index" class="group cursor-pointer">
            <template #image>
              <div class="aspect-video bg-secondary-200" />
            </template>
            <div class="p-4">
              <span class="text-xs text-primary-600 font-medium">{{ article.category }}</span>
              <h3 class="text-lg font-bold text-secondary-900 mt-1 group-hover:text-primary-600">{{ article.title }}</h3>
            </div>
          </UiCard>
        </div>
      </UiContainer>
